...
- [x] `T111` 行业板块历史 endpoint 回退、显式预热、cache 覆盖、用户空态与真实 smoke。
  映射：`AC-020`、`AC-021`
- [x] `T112` 完成 M1 PR #54、CI、clean `main` 验收和 `v3.0.0-alpha.1` annotated tag。

## Batch 2：M2 Fund Profile

- [x] `T201` 冻结 Fund Profile contract、模型和三个 P0 隔离门禁；增加旧表 round-trip 与全量 endpoint 零调用测试义务。
  映射：`AC-012` 至 `AC-014`、`AC-M2-08`、`AC-M2-09`
- [x] `T202` 定义 AKShare 基金库并集、去重/份额规则和 endpoint coverage report。
- [x] `T203` 接入基金目录/概况 mapper，稳定详情 URL。
- [x] `T204` 按“全量 TTL snapshot / 单基金按需”接入申购赎回和费率，保留条件、渠道和原/优惠费率。
...
- [ ] `T208` 三类基金 mandatory live smoke、trace、本地回归和浏览器验收已完成；远端 PR、CI、push 与 `alpha.2` annotated tag 待授权。
