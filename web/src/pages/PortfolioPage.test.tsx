...
const response = {
  ...
  data: {
    ...
    valuation: {
      state: "unavailable",
      label: "当前估值暂不可用",
      description: "尚未取得持仓的可用估值，因此不展示组合总值和收益率。",
    ...
    observations: [
      { tone: "attention", title: "当前估值暂不可用", description: "510300 尚未取得可用估值。" }
    ]
  }
};
...
describe("PortfolioPage", () => {
  ...
  it("separates configured holdings from watchlist and handles missing valuations", async () => {
    ...
    expect(screen.getByText(/当前配置中的持仓/)).toBeInTheDocument();
    expect(screen.getAllByText("当前估值暂不可用").length).toBeGreaterThan(0);
    expect(screen.queryByText("-100.00%")).not.toBeInTheDocument();
    expect(screen.getByText(/尚未取得持仓的可用估值/)).toBeInTheDocument();
    expect(screen.getAllByText("权重待估值")).toHaveLength(2);
    expect(screen.getByText("510300 尚未取得可用估值。")).toBeInTheDocument();
      ...
      vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          ...
          data: {
            ...
            valuation: { state: "complete", label: "估值已齐全", description: "当前持仓均已取得可用估值。", valued_position_count: 1, unvalued_position_count: 0 },
            observations: []
          }
        })
      })
